...
describe('Cookie Helper Utility', () => {
  ...
  test('Must reject cookies without ssid (identity token)', () => {
    expect(() => parseCookieHeader(mockNoIdentity)).toThrow(
      'Cookie does not contain ssid (not logged session).'
    );
  });

  test('Must reject invalid data types', () => {
    expect(() => parseCookieHeader(mockInvalidType)).toThrow('Invalid cookie format.');
  });

  test('Must reject empty cookies string', () => {
    expect(() => parseCookieHeader(mockEmptyString)).toThrow('No cookies were detected.');
  });
});
