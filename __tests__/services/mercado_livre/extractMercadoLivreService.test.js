...
describe('Scraper Service - Unit Tests', () => {
  ...
  test('Must extract product data successfully (happy path)', async () => {
    ...
    const result = await scraperService.fetchProduct('https://meli.la/curto', {
      userId: 'user_123',
      marketplace: 'ML'
    });
  ...
  test('Should throw error if cookies are missing', async () => {
    urlResolutionService.resolveFinalUrl.mockResolvedValue('https://produto.ml/MLB-123');

    userConfigRepository.getUserConfigs.mockResolvedValue({ cookies: [] });

    await expect(
      scraperService.fetchProduct('https://link.com', {
        userId: 'user_sem_cookie',
        marketplace: 'ML'
      })
  ...
  test('Must close context even if navigation fails', async () => {
    ...
    await expect(
      scraperService.fetchProduct('https://link.com', { userId: 'user_123', marketplace: 'ML' })
